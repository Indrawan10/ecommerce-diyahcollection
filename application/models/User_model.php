...
class User_model extends CI_Model
{
    ...
    public function getOrderByInvoice($id)
    {
        ...
        $this->db->select('invoice.*, transaction.*'); // Select all columns from both tables
        $this->db->from('invoice');
        $this->db->join('transaction', 'transaction.id_invoice = invoice.invoice_code'); // Join on invoice_code
        $this->db->where('invoice.invoice_code', $id);
        $this->db->where('invoice.user', $user);
    ...
    {
        $user = $this->session->userdata('id');
        $this->db->select('transaction.*, products.id as productID');
        $this->db->from('transaction');
        $this->db->join('products', 'products.title = transaction.product_name', 'left');
        $this->db->where('transaction.user', $user);
        $this->db->where('transaction.id_invoice', $id);
        return $this->db->get();
    }
