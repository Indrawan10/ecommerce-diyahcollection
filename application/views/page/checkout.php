...
<div class="wrapper">
            ...
            <table class="table">
                ...
                <?php
                 ...
                 foreach($cart->result_array() as $item): 
                    $total += $item['price'] * $this->input->get('qty');
                 ?>
                <tr>
                    <td># <?= $item['title']; ?></td>
                    <td class="text-center"><?= $this->input->get('qty'); ?></td>
                 
                    <td>Rp<?= number_format($item['price'] * $this->input->get('qty'),0,",","."); ?></td>
                    <input type="hidden" value="<?= $item['price'] ?>" name="price">
                    <input type="hidden" value="<?= $item['slug'] ?>" name="slug">
                    <input type="hidden" value="<?= $item['img'] ?>" name="img">
                    <input type="hidden" value="<?= $item['title'] ?>" name="product_name">
                    <input type="hidden" value="<?= $this->input->get('qty'); ?>" name="qty">
                </tr>
                <?php endforeach; ?>
            </table>
        </div>
    ...
    <?php
        ...
        foreach($cart->result_array() as $c){
            $totalall += intval($c['price']) * intval($this->input->get('qty'));
            $totalitem += intval($this->input->get('qty'));
        }
    ?>
