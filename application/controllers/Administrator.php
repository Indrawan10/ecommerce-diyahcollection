...
class Administrator extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        if (!$this->session->userdata('admin') && !$this->session->userdata('owner')) {
    ...
    {
        $data['title'] = $this->session->userdata('admin') ? 'Dashboard - Admin Panel' : 'Dashboard - Owner Panel';
    ...
    public function deleteComplain($id)
    {
        ...
        $this->session->set_flashdata('upload', "<script>
            swal({
            text: 'Complain berhasil dihapus',
            icon: 'success'
            });
            </script>");
        redirect(base_url() . 'administrator/complain');
    }
    ...
    public function search_products()
    {
        ...
        $config['total_rows'] = $this->Products_model->getSearchProducts($key, 0, 0)->num_rows();
        $config['per_page'] = 10;
        $config['first_link']       = 'First';
        $config['last_link']        = 'Last';
        $config['next_link']        = 'Next';
        ...
        // Ambil data dari segment URL untuk pagination
        ...
        // Muat view dengan data yang diperoleh
        $this->load->view('templates/header_admin', $data);
        $this->load->view('administrator/products', $data);
        $this->load->view('templates/footer_admin');
    }
    ...
    public function add_product()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            // Cek apakah judul produk sudah ada
            $title = $this->input->post('title');
            if ($this->Products_model->isTitleExist($title)) {
                $this->session->set_flashdata('failed', "<div class='alert alert-danger' role='alert'>
                Gagal menambah produk karena Judul Produk sudah ada.
              </div>");
    ...
    public function pembelian()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $query = "SELECT A.*,B.nama,C.title FROM pembelian A INNER JOIN supplier B ON A.supplier_id=B.id INNER JOIN products C ON A.product_id=C.id";

            $data['pembelian'] = $this->db->query($query);
    ...
    {
        $data['title'] = 'Complain - Admin Panel';
        $data['complain'] = $this->db->order_by('created_at', 'DESC')->get('complain')->result_array();

        $this->load->view('templates/header_admin', $data);
        $this->load->view('administrator/complain', $data);
        $this->load->view('templates/footer_admin');
    }

    public function laporan_pendapatan()
    {
        $data['title'] = 'Laporan Pendapatan - Admin Panel';

        if (@$_GET['dari']) {
            $dari = $_GET['dari'];
            $sampai = $_GET['sampai'];

            $this->db->select('transaction.product_name, COUNT(transaction.product_name) as total_sold, transaction.price as unit_price, SUM(transaction.price) as total_price, invoice.status')
                ->from('transaction')
                ->join('invoice', 'transaction.id_invoice = invoice.invoice_code')
                ->where('invoice.pay_status', 'settlement')
                ->where('invoice.status', 4)
                ->where("DATE(invoice.date_input) BETWEEN '$dari' AND '$sampai'")
                ->group_by('transaction.product_name, transaction.price, invoice.status');
            $data['pendapatan'] = $this->db->get()->result_array();

            $this->load->view('administrator/cetakpendapatan', $data);
        } else {
            $this->load->view('templates/header_admin', $data);
            $this->load->view('administrator/laporan_pendapatan', $data);
            $this->load->view('templates/footer_admin');
        }
    }
    public function laporan_labarugi()
    {
        $data['title'] = 'Laporan Laba Rugi - Admin Panel';
        ...
        if (@$_GET['dari']) {
            ...
            // Query untuk mengambil data pembelian
           $this->db->select('p.title AS product_name, SUM(pb.qty) AS jumlah_pembelian, SUM(pb.harga * pb.qty) AS total_harga_pembelian')
                ->from('products p')
                ->join('pembelian pb', 'p.id = pb.product_id')
                ->where('pb.status', 'Sudah Bayar')  // Filter berdasarkan status pembayaran
                ->where("DATE(pb.created_at) BETWEEN '$dari' AND '$sampai'")
                ->group_by('p.title');

            $result_pembelian = $this->db->get()->result_array();
          

            // Query untuk mengambil data penjualan
            $query_penjualan = $this->db->select('t.product_name, SUM(t.qty) AS jumlah_terjual, SUM(t.price * t.qty) AS total_harga_penjualan')
                ->from('transaction t')
                ->join('invoice i', 't.id_invoice = i.invoice_code')
                ->where('i.status', 4) 
                ->where("DATE(i.date_input) BETWEEN '$dari' AND '$sampai'")
                ->group_by('t.product_name');

            $result_penjualan = $query_penjualan->get()->result_array();
           
            // Menggabungkan data pembelian dan penjualan berdasarkan nama produk
            $laporan = array();

            // Memasukkan data pembelian ke dalam $laporan
            foreach ($result_pembelian as $row) {
                $laporan[$row['product_name']] = array(
                    'product_name' => $row['product_name'],
                    'jumlah_pembelian' => $row['jumlah_pembelian'],
                    'total_harga_pembelian' => $row['total_harga_pembelian'],
                    'jumlah_terjual' => 0,
                    'total_harga_penjualan' => 0
                );
            }

            // Memasukkan data penjualan ke dalam $laporan
            foreach ($result_penjualan as $row) {
                $product_name = $row['product_name'];
                if (isset($laporan[$product_name])) {
                    $laporan[$product_name]['jumlah_terjual'] = $row['jumlah_terjual'];
                    $laporan[$product_name]['total_harga_penjualan'] = $row['total_harga_penjualan'];
                } else {
                    $laporan[$product_name] = array(
                        'product_name' => $product_name,
                        'jumlah_pembelian' => 0,
                        'total_harga_pembelian' => 0,
                        'jumlah_terjual' => $row['jumlah_terjual'],
                        'total_harga_penjualan' => $row['total_harga_penjualan']
                    );
                }
            }


            $data['result'] = array_values($laporan);

            $this->load->view('administrator/cetaklabarugi', $data);
        } else {
            $this->load->view('templates/header_admin', $data);
            $this->load->view('administrator/laporan_labarugi', $data);
            $this->load->view('templates/footer_admin');
        }
    }
    ...
    public function laporan_penjualan()
    {
        ...
        if (@$_GET['dari']) {
            ...
            $this->db->select('invoice.*, transaction.*')
                ->from('invoice')
                ->join('transaction', 'transaction.id_invoice = invoice.invoice_code')
                ->where('invoice.pay_status', 'settlement')
                ->where('invoice.status', 4)
                ->where("DATE(invoice.date_input) BETWEEN '$dari' AND '$sampai'");
            ...
            $this->load->view('administrator/cetakpenjualan', $data);
        } else {
            $this->load->view('templates/header_admin', $data);
    ...
    {
        $data['title'] = 'Laporan Pembelian - Admin Panel';

        if (@$_GET['dari']) {
            $dari = $_GET['dari'];
            $sampai = $_GET['sampai'];

            $query = "SELECT A.*,B.nama,C.title FROM pembelian A INNER JOIN supplier B ON A.supplier_id=B.id INNER JOIN products C ON A.product_id=C.id WHERE A.status='Sudah Bayar' AND DATE(created_at) BETWEEN '$dari' AND '$sampai'";

            $data['pembelian'] = $this->db->query($query)->result_array();

            $this->load->view('administrator/cetakpembelian', $data);
        } else {
            $this->load->view('templates/header_admin', $data);
    ...
    public function laporan_returcustomer()
    {
        $data['title'] = 'Laporan Retur Customer - Admin Panel';

        if (@$_GET['dari']) {
            $dari = $_GET['dari'];
            $sampai = $_GET['sampai'];

            $this->db->select('retur_penjualan.*, invoice.total_price, invoice.total_all')
                ->from('retur_penjualan')
                ->join('invoice', 'invoice.invoice_code = retur_penjualan.id_invoice')
                ->where('retur_penjualan.status', 'selesai')
                ->where("DATE(retur_penjualan.created_at) BETWEEN '$dari' AND '$sampai'");

            $data['retur'] = $this->db->get()->result_array();

            $this->load->view('administrator/cetakreturcustomer', $data);
        } else {
            $this->load->view('templates/header_admin', $data);
            $this->load->view('administrator/laporan_returcustomer', $data);
            $this->load->view('templates/footer_admin');
        }
    }
    public function laporan_returpembelian()
    {
        $data['title'] = 'Laporan Retur Customer - Admin Panel';
        ...
        if (@$_GET['dari']) {
            ...
            $this->db->select('A.*, B.nama, C.title')
                ->from('retur_pembelian A')
                ->join('supplier B', 'A.supplier_id = B.id')
                ->join('products C', 'A.product_id = C.id')
                ->where('A.status', 'Selesai')
                ->where("DATE(A.created_at) BETWEEN '$dari' AND '$sampai'");

            $data['retur'] = $this->db->get()->result_array();

            $this->load->view('administrator/cetakreturpembelian', $data);
        } else {
            $this->load->view('templates/header_admin', $data);
            $this->load->view('administrator/laporan_returpembelian', $data);
            $this->load->view('templates/footer_admin');
        }
    }
    public function laporan_stokmasuk()
    {
        $data['title'] = 'Laporan Pembelian - Admin Panel';
        ...
        if (@$_GET['dari']) {
            ...
            $data['stok'] = $this->db->query($query)->result_array();

            $this->load->view('administrator/cetakstokmasuk', $data);
        } else {
            $this->load->view('templates/header_admin', $data);
            $this->load->view('administrator/laporan_stokmasuk', $data);
            $this->load->view('templates/footer_admin');
        }
    }

    public function laporan_stokkeluar()
    {
        $data['title'] = 'Laporan Pembelian - Admin Panel';
        ...
        if (@$_GET['dari']) {
            ...
            $query = "SELECT A.*,B.date_input FROM transaction A INNER JOIN invoice B ON A.id_invoice=B.invoice_code WHERE DATE(date_input) BETWEEN '$dari' AND '$sampai'";

            $data['stok'] = $this->db->query($query)->result_array();

            $this->load->view('administrator/cetakstokkeluar', $data);
        } else {
            $this->load->view('templates/header_admin', $data);
            $this->load->view('administrator/laporan_stokkeluar', $data);
            $this->load->view('templates/footer_admin');
        }
    }
    ...
    {
        $data['title'] = 'Retur Customer - Admin Panel';

        $data['retur'] = $this->db->order_by('created_at', 'DESC')->get('retur_penjualan')->result_array();

        $this->load->view('templates/header_admin', $data);
        $this->load->view('administrator/retur_customer', $data);
        $this->load->view('templates/footer_admin');
    }

    public function stsretur($id, $status)
    {
        ...
        if ($status == 'selesai') {
            ...
            $this->db->where('id', $id);
            $retur = $this->db->get('retur_penjualan')->row();

            if ($retur) {
                $product_name = $retur->product_name;
                $qty = $retur->qty;
                $this->db->select('id');
        ...
        redirect(base_url() . 'administrator/returcustomer');
    }

    public function returpembelian()
    {
        $data['title'] = 'Retur Pembelian - Admin Panel';

        $query = "SELECT A.*,B.nama,C.title FROM retur_pembelian A INNER JOIN supplier B ON A.supplier_id=B.id INNER JOIN products C ON A.product_id=C.id";

        $data['retur'] = $this->db->query($query);
        $data['products'] = $this->db->get('products')->result();
        $data['supplier'] = $this->db->get('supplier')->result();

        $this->load->view('templates/header_admin', $data);
        $this->load->view('administrator/returpembelian', $data);
        $this->load->view('templates/footer_admin');
    }
    ...
    public function status_retur($id)
    {
        ...
        redirect(base_url() . 'administrator/returpembelian');
    }

    public function retur($id)
    {
        // Validasi input
        $this->form_validation->set_rules('qty_retur', 'QTY Retur', 'required|numeric', [
            'required' => 'Qty Retur wajib diisi',
            'numeric' => 'Qty Retur harus berupa angka'
        ]);

        if ($this->form_validation->run() == false) {
            $data['title'] = 'Retur Pembelian - Admin Panel';
            $data['pembelian'] = $this->db->get_where('pembelian', ['id' => $id])->row_array();

            // Ambil data produk dan supplier untuk form retur
            $data['products'] = $this->db->get('products')->result();
            $data['supplier'] = $this->db->get('supplier')->result();

            // Load view untuk form retur
            $this->load->view('templates/header_admin', $data);
            $this->load->view('administrator/retur_pembelian', $data);
            $this->load->view('templates/footer_admin');
        } else {
            // Ambil data pembelian untuk mendapatkan supplier_id
            $pembelian = $this->db->get_where('pembelian', ['id' => $id])->row_array();
            if (!$pembelian) {
                // Jika pembelian tidak ditemukan, tampilkan pesan error dan redirect
            ...
            // Siapkan data untuk tabel retur_pembelian
            $data = [
                'pembelian_id' => $id,
                'product_id' => $this->input->post('product_id'),
                'supplier_id' => $pembelian['supplier_id'], // Sertakan supplier_id dari pembelian
                'harga' => intval(str_replace(['Rp.', ','], '', $this->input->post('harga_retur'))),
                'qty' => $this->input->post('qty_retur'),
                'status' => 'Pending', // Status default
                'created_at' => date('Y-m-d H:i:s')
            ];

            // Insert data retur ke tabel retur_pembelian
            $this->db->insert('retur_pembelian', $data);

            // Update kuantitas pembelian
            $this->db->set('qty', 'qty-' . $this->input->post('qty_retur'), FALSE);
            $this->db->where('id', $id);
            $this->db->update('pembelian');

            // Update stok produk
            $this->db->set('stock', 'stock-' . $this->input->post('qty_retur'), FALSE);
    ...
    {
        // Ambil data retur dan join dengan tabel pembelian, produk, dan supplier
        $this->db->select('retur_pembelian.*, pembelian.supplier_id, supplier.nama as nama_supplier, products.title as nama_produk');
        $this->db->from('retur_pembelian');
        $this->db->join('pembelian', 'retur_pembelian.pembelian_id = pembelian.id');
        $this->db->join('supplier', 'pembelian.supplier_id = supplier.id');
        $this->db->join('products', 'retur_pembelian.product_id = products.id');
        $this->db->where('retur_pembelian.pembelian_id', $pembelian_id);
        $data['retur'] = $this->db->get()->result_array();

        // Load view untuk menampilkan data retur
        $data['title'] = 'Data Retur Pembelian';
        $this->load->view('templates/header_admin', $data);
        $this->load->view('administrator/data_retur_pembelian', $data);
        $this->load->view('templates/footer_admin');
    }
    ...
    public function pembelians($id)
    {
        $this->form_validation->set_rules('harga', 'Harga', 'required', ['required' => 'Harga wajib diisi']);
        if ($this->form_validation->run() == false) {
            $data['title'] = 'Edit Pembelian - Admin Panel';
            $data['pembelian'] = $this->db->get_where('pembelian', ['id' => $id])->row_array();
            ...
            $this->load->view('templates/header_admin', $data);
            $this->load->view('administrator/edit_pembelian', $data);
            $this->load->view('templates/footer_admin');
        } else {
            ...
            $this->session->set_flashdata('upload', "<script>
                swal({
                text: 'Pembelian berhasil diubah',
                icon: 'success'
                });
                </script>");
            redirect(base_url() . 'administrator/pembelian');
    ...
    public function bayar($id)
    {
        ...
        if ($_POST) {
            ...
            if ($data['bayar'] < $data['subtotal']) {
                ...
                redirect(base_url() . 'administrator/bayar/' . $id);
            }

            $this->db->set('status', 'Sudah Bayar');
            $this->db->set('qty', $data['qty']);
            $this->db->set('bayar', $data['bayar']);
            $this->db->where('id', $id);
            $this->db->update('pembelian');
            ...
            $data['title'] = 'Bayar Pembelian - Admin Panel';
            $data['pembelian'] = $this->db->get_where('pembelian', ['id' => $id])->row_array();
            ...
            $this->load->view('templates/header_admin', $data);
            $this->load->view('administrator/bayar_pembelian', $data);
            $this->load->view('templates/footer_admin');
        }
    }
